...
if (!isset($_GET) || empty($_GET)) {
    ...
    $meses = implode(",", $_GET['mes']);
}
//print_r($meses);
$abelha = CallAPI('GET', 'http://localhost:80/api/floracao/[' . $meses . ']');
...
<?php
foreach ($abelha as $key => $mes) {
//    print_r($mes);
    ?>
    <span class="nome"><?= $mes->mes_nome ?></span>
    <ul>
        <?php
        foreach (explode(', ', $mes->plantas) as $planta) {
            ?>
            <li><span class="nome_cientifico"><?= $planta ?></span></li>
